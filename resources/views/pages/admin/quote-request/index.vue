    ...
    <div class="mt-5 divide-y">
        <div class="hover:drop-shadow-md p-2 py-3 flex justify-between" v-for="quote in quotes.data" :key="quote.id" :class="[!quote.status?'bg-gray-50':'']">
            <Link :href="route('quote-request.show',quote.id)" class="text-sm flex gap-2" :class="[!quote.status?'text-gray-900':'text-gray-400']">
                ...
                <svg v-if="quote.status===2" class="h-4 fill-teal-700 self-center" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M243.8 339.8C232.9 350.7 215.1 350.7 204.2 339.8L140.2 275.8C129.3 264.9 129.3 247.1 140.2 236.2C151.1 225.3 168.9 225.3 179.8 236.2L224 280.4L332.2 172.2C343.1 161.3 360.9 161.3 371.8 172.2C382.7 183.1 382.7 200.9 371.8 211.8L243.8 339.8zM512 256C512 397.4 397.4 512 256 512C114.6 512 0 397.4 0 256C0 114.6 114.6 0 256 0C397.4 0 512 114.6 512 256zM256 48C141.1 48 48 141.1 48 256C48 370.9 141.1 464 256 464C370.9 464 464 370.9 464 256C464 141.1 370.9 48 256 48z"/></svg>
            ...
            <div class="flex gap-2">
                <Link v-if="!quote.status" :href="route('quote.responded',quote.id)" class="text-sm font-bold  text-start text-teal-700"  method="patch" as="button">
                    Mark responded
                </Link>

                <Link :href="route('quote-request.destroy',quote.id)" class="text-sm font-bold  text-start text-red-700"  method="delete" as="button">
                   Delete
                </Link>
            </div>
        </div>
    </div>
    <pagination :data="quotes"></pagination>
</admin>
